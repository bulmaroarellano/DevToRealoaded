...
postRef.on("value", (snapshot) => {
    ...
    console.log(postCollection[0].user.name)
     //postCollection.forEach( (item, index) =>{
        // let postUserRef = database.ref(`/db-devto/${index}/user`);
      //postUserRef.on("value", snapshot => {
        //  let userSanpshot= snapshot.val()
          //console.log(userSanpshot)
          //let {name, profile_image} = userSanpshot
  
    for (item in postCollection) {
      let { readable_publish_date, reading_time_minutes, title, cover_image, name} = postCollection[item];
      $(".cardContainer").append(`
      <div class="card mt-3 br-post post-card">
          <div class="card-body">
              <div class="d-flex c-header">
                      <img src="${cover_image}" alt="" class="br-100 pad">
